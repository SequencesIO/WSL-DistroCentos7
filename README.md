Forked from the original Microsoft reference implementation for a linux distribution installer.  Modifications were made to install CentOS 7 and change the windows app name to "centos7".  Below is the original README, but all commands when run with this project would be "centos7" instead of "launcer".
